...
namespace utilities {
    ...
    template<typename _TyCandidate, typename _Ty> static constexpr inline
        typename std::enable_if<std::is_integral<_TyCandidate>::value && std::is_integral<_Ty>::value, bool>::type __attribute__((__always_inline__))
    ...
    template<typename _TyCandidate, typename _TyFirst, typename... _TyList> static constexpr inline
        typename std::enable_if<std::is_integral<_TyCandidate>::value && std::is_integral<_TyFirst>::value, bool>::type __attribute__((__always_inline__))
        is_in(const _TyCandidate& candidate, const _TyFirst& first, const _TyList&... list) noexcept {
        return candidate == first || utilities::is_in(candidate, list...);
    }

    template<typename _TyValue, typename _TyLow, typename _TyHigh> static inline constexpr
        typename std::enable_if<std::is_arithmetic<_TyValue>::value && std::is_arithmetic<_TyLow>::value && std::is_arithmetic<_TyHigh>::value, bool>::type
    ...
    namespace rgb {
        ...
        namespace transformers {
            ...
            static constexpr inline void __attribute__((__always_inline__)) binary(RGBQUAD& pixel) noexcept {
                pixel.rgbBlue = pixel.rgbGreen = pixel.rgbRed = (static_cast<double>(pixel.rgbBlue) + pixel.rgbGreen + pixel.rgbRed) / 3.0 >= 128.0 ? 255 : 0;
            }
        ...
        namespace removers { // this is really verbose but makes mutating the pixel buffers possible with a single std::for_each call
            ...
            template<> struct zero<RGB::RED> final {
                    constexpr inline void __attribute__((__always_inline__)) operator()(RGBQUAD & pixel) const noexcept { pixel.rgbRed = 0; }
            };

            template<> struct zero<RGB::GREEN> final {
                    constexpr inline void __attribute__((__always_inline__)) operator()(RGBQUAD & pixel) const noexcept { pixel.rgbGreen = 0; }
            };

            template<> struct zero<RGB::BLUE> final {
                    constexpr inline void __attribute__((__always_inline__)) operator()(RGBQUAD & pixel) const noexcept { pixel.rgbBlue = 0; }
            };

            template<> struct zero<RGB::REDGREEN> final {
                    constexpr inline void __attribute__((__always_inline__)) operator()(RGBQUAD & pixel) const noexcept { pixel.rgbRed = pixel.rgbGreen = 0; }
            };

            template<> struct zero<RGB::GREENBLUE> final {
                    constexpr inline void __attribute__((__always_inline__)) operator()(RGBQUAD & pixel) const noexcept { pixel.rgbGreen = pixel.rgbBlue = 0; }
            };

            template<> struct zero<RGB::REDBLUE> final {
                    constexpr inline void __attribute__((__always_inline__)) operator()(RGBQUAD & pixel) const noexcept { pixel.rgbRed = pixel.rgbBlue = 0; }
            };
    ...
    namespace crc {
        ...
        [[nodiscard]] static constexpr inline unsigned __attribute__((__always_inline__)) calculate(
            ...
            if (!bytestream) {
                ::fprintf(stderr, "Error in %s, received a null buffer\n", __PRETTY_FUNCTION__);
                return 0;
            }
