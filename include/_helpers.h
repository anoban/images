...
enum RGB_TAG : unsigned char { RED, GREEN, BLUE, REDGREEN, REDBLUE, GREENBLUE };

// mechanism to be used in converting the pixels to black and white
enum BW_TRANSFORMATION : unsigned char { AVERAGE, WEIGHTED_AVERAGE, LUMINOSITY, BINARY };
...
static void __stdcall average(_Inout_ RGBQUAD* const pixel) {
    pixel->rgbBlue = pixel->rgbGreen = pixel->rgbRed = (((long double) (pixel->rgbBlue) + pixel->rgbGreen + pixel->rgbRed) / 3.0L);
}

static void __stdcall weighted_average(_Inout_ RGBQUAD* const pixel) {
    pixel->rgbBlue = pixel->rgbGreen = pixel->rgbRed = (pixel->rgbBlue * 0.299L + pixel->rgbGreen * 0.587L + pixel->rgbRed * 0.114L);
}

static void __stdcall luminosity(_Inout_ RGBQUAD* const pixel) {
    pixel->rgbBlue = pixel->rgbGreen = pixel->rgbRed = (pixel->rgbBlue * 0.2126L + pixel->rgbGreen * 0.7152L + pixel->rgbRed * 0.0722L);
}

// every colour value gets scaled down to 0 or scaled up to 255 depending on the average value of colours in a pixel
static void __stdcall binary(_Inout_ RGBQUAD* const pixel) {
    pixel->rgbBlue = pixel->rgbGreen = pixel->rgbRed =
        ((double) (pixel->rgbBlue) + pixel->rgbGreen + pixel->rgbRed) / 3.0 >= 128.0 ? 255 : 0;
}

// each colour value gets scaled down to 0 or scaled up to 255 depending on the corresponding value
static void __stdcall negative(_Inout_ RGBQUAD* const pixel) {
...
static const unsigned CRC32_LOOKUPTABLE_IEEE[256] {
...
class complex final { // doesn't provide the arithmetic functionalities like stdcomplex<> though
    ...
    public:
        ...
        explicit complex(const _Ty& v) : __x { v }, __y { v } { }

        complex(const _Ty& x, const _Ty& y) : __x { x }, __y { y } { }
